create table if not exists public.room_data_backup_sets (
...
create table if not exists public.room_data_backup_rows (
  id uuid primary key default uuid_generate_v4(),
  backup_set_id uuid not null references public.room_data_backup_sets(id) on delete cascade,
  table_name text not null,
  chunk_index int not null default 0,
  snapshot_rows jsonb not null default '[]'::jsonb,
  row_count int not null default 0,
  created_at timestamptz not null default now(),
  unique (backup_set_id, table_name, chunk_index)
);

alter table public.room_data_backup_sets enable row level security;
alter table public.room_data_backup_rows enable row level security;

create policy "admins read backup sets" on public.room_data_backup_sets
for select to authenticated
using (public.current_user_role() = 'admin');

create policy "admins manage backup sets" on public.room_data_backup_sets
for all to authenticated
using (public.current_user_role() = 'admin')
with check (public.current_user_role() = 'admin');

create policy "admins read backup rows" on public.room_data_backup_rows
for select to authenticated
using (public.current_user_role() = 'admin');

create policy "admins manage backup rows" on public.room_data_backup_rows
for all to authenticated
using (public.current_user_role() = 'admin')
with check (public.current_user_role() = 'admin');
