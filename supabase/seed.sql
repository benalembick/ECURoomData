...
insert into public.room_patterns (category_id, name, description, ecu_av_patterns, vizcom_av_patterns, default_booking_rules, default_o365_config, timetabling_eligible, access_logic, required_attribute_keys, approval_requirements, downstream_system_codes)
select c.id, p.name, p.description, p.ecu_av_patterns, p.vizcom_av_patterns, p.booking::jsonb, p.o365::jsonb, p.timetable, p.access::jsonb, p.required, p.approvals::jsonb, p.systems
from public.room_categories c
join (values
  ('Teaching Space', 'Standard Teaching Room', 'General purpose teaching space.', array['Standard Teaching','Standard Teaching Room','ECU Standard Teaching'], array['STD-TEACH','Teaching Standard'], '{"staff_bookable": false, "student_bookable": false}', '{"room_finder_category": "Teaching"}', true, '{"staff": "timetabled", "students": "class enrolment"}', array['projector','lecture_capture'], '[{"stage":1,"role":"approver"},{"stage":2,"role":"system_owner"}]', array['O365','ARCHIBUS','TIMETABLING','APPSPACE','MAINTENANCE']),
  ('Teaching Space', 'TEAL Space', 'Technology enhanced active learning room.', array['TEAL','TEAL Space','Active Learning'], array['TEAL','TEAL-AV','Active Learning Studio'], '{"staff_bookable": false, "student_bookable": false}', '{"room_finder_category": "Teaching"}', true, '{"students": "class enrolment", "support": "AV escalation"}', array['teams_enabled','lecture_capture','specialist_equipment'], '[{"stage":1,"role":"approver"},{"stage":2,"role":"system_owner"}]', array['O365','ARCHIBUS','TIMETABLING','APPSPACE','MAINTENANCE']),
  ('Meeting Room', 'Meeting Room', 'Staff meeting room with optional O365 mailbox.', array['Meeting','Meeting Room','Teams Meeting'], array['MEET','MTR Standard'], '{"staff_bookable": true, "student_bookable": false}', '{"room_finder_category": "Meeting"}', false, '{"staff": "all staff"}', array['teams_enabled'], '[{"stage":1,"role":"system_owner"}]', array['O365','ARCHIBUS','APPSPACE']),
  ('Student Collaboration', 'Student Project Room', 'Student project and workspace room.', array['Student Project','Student Collaboration','Workspace'], array['STUDENT-PROJECT','Workspace Display'], '{"staff_bookable": true, "student_bookable": true}', '{"room_finder_category": "Workspace"}', false, '{"students": "student booking group"}', array['student_bookable'], '[{"stage":1,"role":"approver"},{"stage":2,"role":"system_owner"}]', array['O365','SECURITY','ARCHIBUS','APPSPACE']),
  ('WAAPA Specialist', 'Specialist WAAPA Space', 'Specialist creative production or practice space.', array['WAAPA Specialist','Practice Room','Specialist Creative'], array['WAAPA-SPEC','Practice Room AV'], '{"staff_bookable": true, "student_bookable": false}', '{"room_finder_category": "Specialist"}', true, '{"students": "approved cohort", "staff": "WAAPA staff"}', array['specialist_equipment','student_access_group'], '[{"stage":1,"role":"approver"},{"stage":2,"role":"system_owner"}]', array['O365','ARCHIBUS','TIMETABLING','SECURITY','MAINTENANCE'])
) as p(category_name, name, description, ecu_av_patterns, vizcom_av_patterns, booking, o365, timetable, access, required, approvals, systems) on p.category_name = c.name
on conflict do nothing;
